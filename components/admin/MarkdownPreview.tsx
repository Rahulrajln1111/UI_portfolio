...
const CodeBlock = ({ inline, className, children, ...props }: any) => {
    ...
    const handleCopy = () => {
        ...
        if (codeToCopy.length === 0) {
             toast({ title: "Copy Failed", description: "The content to copy was empty.", variant: "destructive" });
             return;
        }
        navigator.clipboard.writeText(codeToCopy)
            .then(() => toast({ title: "Copied!", description: `Code block (${language}) copied to clipboard.`, variant: "default" }))
        ...
        <div className={className}>
            <ReactMarkdown
                ...
                components={{
                    ...
                    // 🔥 FIXED HEADINGS: preserve IDs from rehype-slug
                    h2: ({ node, ...props }) => (
                        <h2
                            {...props}
                    ...
                    h3: ({ node, ...props }) => (
                        <h3
                            {...props}
                            className="text-2xl font-semibold mt-6 mb-3 text-white scroll-mt-24"
                        />
                    ),
